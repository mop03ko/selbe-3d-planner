...
import {loadLayout,STORE_KEY,LEGACY_STORE_KEY,History,validateLayout,itemWarnings,pointInPoly} from './model.js';
...
<main class="workspace">
 <aside class="left-panel" aria-label="Давхар ба өрөөнүүд">
  ...
  <div class="left-bottom"><a href="./renders.html" class="render-gallery-link"><img src="./renders/imagegen/living-thumb.webp" alt="" width="64" height="48"/><span>Интерьерийн зургууд<small>15 өрөө · шинэ рендерүүд</small></span>${icon('arrow')}</a><button id="inspiration" class="inspiration-button">${icon('palette')}<span>Интерьерийн санаанууд</span>${icon('arrow')}</button><button id="mobile-import" class="quiet mobile-import">${icon('upload')}Хувилбар оруулах</button><button id="guide" class="quiet">${icon('help')}Хэрхэн ашиглах вэ?</button><button id="source" class="source-note">PDF тулгалт · 2026.09.23 ${icon('arrow')}</button></div>
 </aside>
 <section class="stage" aria-label="3D ажлын талбар">
  ...
  <div class="scene-caption"><span class="eyebrow">СЭЛБЭ / <span id="floor-caption">01</span></span><h2 id="scene-title">Нэгдүгээр давхар</h2><span id="scene-meta">Бүх өрөөний зохион байгуулалт</span><a id="room-render-link" class="room-render-link" href="./renders.html" hidden>${icon('camera')}Энэ өрөөний зураг ${icon('arrow')}</a></div>
...
<nav class="mobile-nav" aria-label="Гар утасны удирдлага"><button data-mobile="rooms">${icon('menu')}Өрөөнүүд</button><button data-mobile="scene" class="active">${icon('cube')}3D харах</button><button data-mobile="edit">${icon('palette')}Тохируулах</button><a class="mobile-gallery-link" href="./renders.html">${icon('camera')}Рендерүүд</a></nav>
...
function showInspiration(){showDialog('Өнгө, материалын хувилбар',`<p>Өнгийг сонгоход 3D загварт шууд харагдана. Цонх, хаалга, тавилгын байрлалыг хэвээр хадгална.</p><div class="theme-list">${Object.entries(THEMES).map(([id,t])=>`<button class="theme-card" data-inspire="${id}"><span class="theme-swatches">${[t.wall,t.wood,t.fabric].map(c=>`<i style="background:${c}"></i>`).join('')}</span>${t.name}</button>`).join('')}</div><p><a href="./renders.html?mode=3d&layout=saved">Одоогийн хувилбарын 3D зургуудыг үзэх ↗</a></p>`);$('#modal-body').querySelectorAll('[data-inspire]').forEach(b=>b.onclick=()=>{layout.theme=b.dataset.inspire;const t=THEMES[layout.theme];for(const r of ROOMS){layout.surfaces[r.id].wall=t.wall;if(!r.tile&&!r.outdoor&&!r.finish)layout.surfaces[r.id].floor=t.floor;}layout.items.forEach(i=>delete i.color);tab='finishes';$('#modal').close();commit();});}
...
catch(error){console.error(error);$('#loading').innerHTML=`${icon('cube')}<strong>3D дүрслэл ассангүй.</strong><span>WebGL дэмждэг Chrome эсвэл Edge хөтөчөөр нээж үзээрэй.</span><a class="primary" href="./renders.html">Интерьерийн зургуудыг үзэх ↗</a><button class="outline" onclick="location.reload()">Дахин ачаалах</button>`;}
renderRooms();renderEditor();renderHistory();
try{if(storage.getItem(LEGACY_STORE_KEY)&&!storage.getItem(STORE_KEY)){
 const note=document.createElement('div');note.className='revision-note';note.innerHTML='<strong>PDF-тэй тулгасан шинэ суурь</strong><span>Өмнөх хадгалсан хувилбар тань хадгалагдсан.</span><button id="download-legacy">Өмнөх хувилбар татах</button>';
 $('.left-bottom').prepend(note);$('#download-legacy').onclick=()=>download(storage.getItem(LEGACY_STORE_KEY),'selbe-layout-before-audit.json','application/json');
}}catch{}
...
$('#source').onclick=()=>showDialog('Төлөвлөлтийн эх сурвалж',`<div class="source-details"><span class="eyebrow">SELBE AJLIIN ZURAG.PDF</span><h3>4–6 хүний гэр бүлийн сууц</h3><p>БА-1, 3/4, 7–13, 16–20 хуудсыг тулгаж шинэчилсэн. I давхрын тавилга БА-1 схемийг дагана. II давхрын тавилга санал болгосон байрлалтай. Дотор талбай: 180.96 м². Террас: 57.92 м², тагт: 10.60 м²; эдгээрийн талбайг эх зурагт 30%-иар жигнэсэн.</p><p>Өрөөний талбайг эх хүснэгтээс авсан. 3D загвар нь схемчилсэн концепц бөгөөд барилга угсралт, тавилга үйлдвэрлэлийн хэмжээсийн зураг биш. Хана, хаалга, цонх, шалны хил нь суурь төлөвлөлтөөр тогтмол.</p><p>Давхцлын анхааруулга нь тавилгын хэвтээ хүрээ болон хаалганы ойр орчмыг шалгана. Явах замын өргөн, бүрэн хаалганы нээлт, инженерийн шугам, аюулгүй байдлын нормыг баталгаажуулахгүй. Усны цэг, гал тогооны байрлалыг өөрчлөхөд газар дээр нь шалгуулна.</p><h3>Эхэд зөрсөн хэмжээсүүд</h3><p>Ц-4: фасад ба түүврийн 1500×1800 мм-ийг ашиглав. ГХ-1: 900×2400, ГХ-2: 1500×2800, ГШХ-1: 2400×2400 мм нь рендерийн түр суурь; үйлдвэрлэлийн баталгаат хэмжээ биш. Террас: байгуулалтын 57.92 м² (хэсэглэлд 56.65). Шилний рамны өнгийг түүврийн хар сааралаар авсан.</p><p>Хана, нээлхийн зарим байрлал схемчилсэн; талбайг PDF хүснэгтээс харуулна. Рендер нь энэ 3D загвараас шууд үүснэ.</p><a href="./plan-notes.html" target="_blank" rel="noopener">Цонхны хүснэгт, эхийн зөрүүг үзэх ↗</a><p>Хувилбарууд зөвхөн таны хөтөчид хадгалагдана. GitHub Pages дээр хувийн хувилбар автоматаар хуваалцахгүй.</p></div>`);
